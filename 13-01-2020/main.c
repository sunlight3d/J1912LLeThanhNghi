...
void find(struct Employee* employees, int N) {
	struct Employee* highestEmployees = malloc(N * sizeof(struct Employee));
	float maxSalary = 0.0;
	int i;
	for(i = 0; i < N; i++){
		struct Employee* ptr = employees + i;
		float monthly_salary = ptr->daily_salary * (float)ptr->working_days;
		if(monthly_salary > maxSalary) {
			maxSalary = monthly_salary;
		}
	}
	//Den day la minh co maxSalary roi
	int j = 0;
	for(i = 0; i < N; i++){
		struct Employee* ptr = employees + i;
		float monthly_salary = ptr->daily_salary * (float)ptr->working_days;
		if(monthly_salary == maxSalary) {
			*(highestEmployees + j) = *(ptr);
			j++;
		}
	}
	printEmployees(highestEmployees, j);
}
void save(struct Employee* employees, int N) {
	char fileName[250];//Include detail path
    printf("Enter file name to save: "); scanf("%s", fileName);
...
void open(struct Employee* employees, int N) {
	char fileName[250];
    ...
    if (!file) {
        ...
        fread(employees, sizeof(struct Employee*), N, file);
        printEmployees(employees, N);
        fclose(file);
    }
}
